...
//   at Urbana-Champaign.
//
// Plosive analysis Stan code (Section B.6)
...
data {
  ...
  int<lower=0> M_2;  // number of item members
  int<lower=0> Q_2;  // number of item effects per member
...
parameters {
  ...
  matrix[Q_2, M_2] gamma_2_raw;
  vector<lower=0>[Q_2] sigma_2_raw;
  cholesky_factor_corr[Q_2] omega_2_raw;
...
transformed parameters {
  ...
  vector<lower=0>[Q_2] sigma_2;  // sd in the item effects
  ...
  sigma_2[1] = sc_q0 * sigma_2_raw[1];
  sigma_2[2:Q_2] = sc_qs * sigma_2_raw[2:Q_2];
  coef[cindx[2, 1]:cindx[2, 2]]
    = to_vector(rep_matrix(sigma_2, M_2)
      .* (omega_2_raw * gamma_2_raw));
...
model {
  ...
  to_vector(gamma_2_raw) ~ normal(0, 1);
  sigma_2_raw ~ normal(0, 1);
  omega_2_raw ~ lkj_corr_cholesky(eta_q);
...
generated quantities {
  ...
  matrix[M_2, Q_2] gamma_2;  // item effects
  matrix[Q_2, Q_2] omega_2;  // correlation in the item effects
  ...
  gamma_2 = vec_to_mat_by_row(M_2, Q_2, coef[cindx[2, 1]:cindx[2, 2]]);
  omega_2 = tcrossprod(omega_2_raw);
}
